...
const YaleLogo: React.FC<{ className?: string }> = ({ className = '' }) => (
  ...
  >
    <defs>
      <linearGradient id="yale-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
        <stop offset="0%" stopColor="#003D6B"/>
        <stop offset="100%" stopColor="#0066CC"/>
      </linearGradient>
    </defs>
    <rect width="120" height="40" rx="6" fill="url(#yale-gradient)"/>
    <text x="60" y="24" textAnchor="middle" fill="white" fontSize="16" fontWeight="bold" fontFamily="Arial, sans-serif">
      YALE
    </text>
  </svg>
);

// Generic Smart Lock Logo component (fallback)
const GenericLockLogo: React.FC<{ className?: string }> = ({ className = '' }) => (
  <svg 
    className={className}
    viewBox="0 0 120 40" 
    xmlns="http://www.w3.org/2000/svg"
  >
    <defs>
      <linearGradient id="generic-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
        <stop offset="0%" stopColor="#4A5568"/>
        <stop offset="100%" stopColor="#718096"/>
      </linearGradient>
    </defs>
    <rect width="120" height="40" rx="6" fill="url(#generic-gradient)"/>
    <text x="60" y="24" textAnchor="middle" fill="white" fontSize="12" fontWeight="bold" fontFamily="Arial, sans-serif">
      SMART LOCK
    </text>
  </svg>
);

// Manufacturer logo selector function
const getManufacturerLogo = (device: SeamDevice): React.FC<{ className?: string }> => {
  const manufacturer = device.properties.model?.manufacturer_display_name?.toLowerCase();
  
  if (manufacturer?.includes('schlage')) {
    return SchlageLogo;
  } else if (manufacturer?.includes('yale')) {
    return YaleLogo;
  } else {
    // Log unknown manufacturers for debugging
    if (manufacturer && manufacturer !== 'unknown') {
      console.log(`[SeamSecurity] Unknown manufacturer: ${manufacturer}, using generic logo`);
    }
    return GenericLockLogo;
  }
};
      ...
      <div className="w-full max-w-full bg-gradient-to-br from-[#0e1726]/95 to-[#0a1120]/95 backdrop-blur-lg rounded-xl shadow-xl border border-[#1e293b]/40 p-4">
        <div className="flex flex-col items-center justify-center text-center space-y-4 py-8">
          <div className="w-16 h-16 flex items-center justify-center rounded-full bg-blue-900/30 mb-2">
            <UilShieldCheck className="w-8 h-8 text-blue-400" />
          </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="bg-[#1e293b] py-1 px-3 rounded-full flex items-center">
                      {React.createElement(getManufacturerLogo(device), { className: "h-4 w-auto" })}
                    </div>
